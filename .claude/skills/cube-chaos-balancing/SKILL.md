...
4. **If the sample behind an existing range looks meaningfully out of date while doing real work here** (e.g. a new mod has added enough `IDENT` cubes that the recorded N below is clearly stale), don't silently trust the old range, and don't silently launch a full corpus-wide recompute either — that's a bigger job than the one balancing question in front of you. **Flag the drift and propose a refresh to the user**, same as any other significant discrepancy. (`cube-chaos-doc-audit` also catches this class of drift on its own periodic pass — a live catch here is a bonus, not the only safety net.)
...
**Pricing a "spawner" cube by decomposition when no single analog fits.** When a cube's main value is that it creates/becomes another unit (plus does something itself), price it as **(notional value of the spawned unit) + (the cube's own body/effect value)**, rather than hunting for one analog that does the whole thing. Real usage, the Unholy mod: `Plague_Imp` (an Imp-equivalent body + a death-poison rider) was priced ≈ 25 (Imp's spawn-value anchor) + rider ≈ 32. The spawned unit's "notional value" is itself a judgment call (a `TOKEN` has no `IDENT` to read it off — see the TOKEN section below), so anchor it once with the user and reuse it across every cube that spawns that unit, per this skill's write-it-down discipline.

**Correction, `cube-chaos-audit` pass, 2026-07-29: the ≈38 `Damned_Soul` figure once cited here was stale.** An earlier draft of this note derived `Damned_Soul` (`TOKEN`, dies leaving an Imp) at ≈25 + ~13 body ≈ 38 by the decomposition method above, but the user has since confirmed the cube's actual shipped manacost (`CUBE: Damned_Soul 5 0 0`, i.e. **5**) is the intentional, correct value — not a regression. Don't re-derive or "fix" `Damned_Soul`'s manacost from the decomposition formula; 5 is the standing anchor now. General lesson: a spawn-value decomposition estimate is a starting suggestion, not a locked price — if the user's shipped number diverges from it, treat the shipped number as authoritative and correct this skill rather than flagging the cube in a future audit.
