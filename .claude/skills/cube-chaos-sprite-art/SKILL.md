...
yourself before looking at a preview card — and `python3 .claude/skills/cube-chaos-mod-setup/scripts/render_workshop_image.py`
too if the sheet holds the mod's namesake perk tile** (that tile is the source of `GameData/<Mod>/Image.png`; see
"Base class/species icon style" below). `.claude/hooks/regen-preview-cards.sh` and
...
**`render_preview_cards.py` honours a real upgrade sheet as of 2026-08-04 — before that it silently didn't, so check the card, not just the sheet.** The script originally hardcoded the sprite-less assumption: it read `<Prefix>_UpgradePerks.c.txt` for *blocks only* and always resolved each upgrade's icon by walking `IsUpgradeFrom:` back to a base perk with a real slot. So drawing a `<Prefix>_UpgradePerks.c.png` produced a correct in-game icon but a preview card still showing the **base perk's** art — the same class of silent, looks-fine-but-wrong output as a stale card. `build_perks()` now registers that sheet as a normal icon source when the `.c.png` exists (each upgrade owning its own slot by file order in the upgrade file), and falls back to the chain walk only when it doesn't, so both conventions work. Broker's `Downtown` (2026-08-04) is the first upgrade perk in this repo with its own art and is the regression test for it: its card must show a dense skyline, not `Rising_Prominence`'s three-tower staircase. **When adding a mod's first-ever upgrade sheet, open the generated card and confirm it differs from the base perk's** — that one look is the only thing separating "the sheet works" from "the sheet is being ignored."
...
**Draw every frame as a FULL 15×15 rendition of the whole cube — not just the pixels that move — and give the strip
the same 1px magenta `#ff006e` guide ring per tile that a main sheet gets.** The frame is composited via
`OverrideWithBIfEqualC(Frame, OwnerCG.Base)` (`EffectType 0`, universal), which rewrites every pixel still matching
the cube's original art with that frame's pixel — so a frame is a *replacement* image, and anything you leave out of
it gets erased rather than left alone. Background/transparent is the same `#0094ff` key the main sheets use, which
is also how a frame legitimately *removes* pixels (the closed half of `Rocket_Silo`'s hatch, `Hell_Portal`'s
suppressed outer flares) or adds them outside the base silhouette. Verified by reading real strips back pixel by
pixel: base game `Main/Sprites/Animations/Acidic_Acid.png` (both frames are the entire acid tile) and this repo's
own `DJ/Sprites/Animations/Speaker_Beat.png` (four full copies of the speaker at different offsets); both carry the
magenta ring on every tile, identical to `Unholy_Cubes.c.png`'s.

**Practical procedure for adding one:** generate the strip in a scratchpad script that reads the cube's real tile
out of its `.c.png` as frame source (so the resting frame is pixel-identical to the shipped icon by construction,
not by hand-copying), render every frame back as an enlarged contact sheet and *look at it* before installing, then
copy into `GameData/<Mod>/Sprites/Animations/`. The whole strip is throwaway-regenerable, so the scratchpad script
is the artefact worth iterating on, not the PNG.
...
**This tile is also the mod's public face, at 20× magnification — draw it accordingly.** A mod's namesake perk tile at slot `(0,0)` is the sole source for `GameData/<Mod>/Image.png`, its Steam Workshop thumbnail *and* the icon shown in the repo-root README's mod table and the mod's own README header. `.claude/hooks/regen-preview-cards.sh` regenerates it automatically, so editing this tile with Write/Edit needs no extra step — but a **script-written** sheet bypasses that hook (see "Script-written sheets don't trigger the regen hooks" above, which applies to `Image.png` exactly as it does to `Preview/`), so run `python3 .claude/skills/cube-chaos-mod-setup/scripts/render_workshop_image.py` by hand after one. Recipe and rationale are owned by `cube-chaos-mod-setup/references/workshop-publishing.md` — don't restate them here.
...
- **A `CUBE` with a `TRIGGER`-type `Animation:` line also gets a looping `.gif` next to its static card** (`<ModPrefix>_Cubes_<CubeName>_<AnimName>.gif`, added 2026-07-30), so a README can actually show the flourish instead of just a static icon. `build_cube_animation_gifs()` reads the frame order/speed straight from the real `Animation:` line (see `cube-chaos-scripting`'s `cube-animation.md` for why the LAST frame is the idle pose, not frame 0, and why each flourish frame's duration is `Thresholds[i+1] * 16ms`) — the only liberty taken is compressing the idle pause between triggers to a fixed `GIF_IDLE_REST_MS` instead of the cube's real (often many-second) cadence, so the loop stays watchable. `TRIGGER`, `CLOCK` and `DOUBLE` are all handled (see the next two bullets); `HP`/`BOOLEAN`/`TIME` would each still need their own playback logic if a mod ever uses one. **README convention: a compact `<table>` right after the cube's static card, 2 animation entries per row** — each entry is a (caption, gif) pair (e.g. `On Cube Creation` for DJ's `Speaker`, whose animation is tied to the ability that creates its Note), `valign="middle"` on every cell, and a new row starts after every 2nd entry (a mod with an odd number of animated cubes just leaves the last row's second pair absent — don't pad it with an empty cell). **No inline `style=` attribute for borders/padding — GitHub's README sanitizer strips the `style` attribute entirely, so any attempt to suppress or customize cell borders (e.g. `border: none`, a single divider line between the two entry-pairs) silently does nothing; what actually renders is GitHub's own default `.markdown-body table` CSS, which always draws a full grid line around every cell of every `<table>`, with no author override available.** Confirmed 2026-07-30: an earlier version of this doc/README specified inline `border-collapse`/`border: none` styling for an outline-only look, which looked right when read as raw HTML but rendered with full grid lines once pushed to GitHub. Plain unstyled `<table>`/`<tr>`/`<td valign="middle">` is now the deliberate convention — don't re-attempt CSS-based border control here, it cannot work on GitHub. **Generate the gif AT the size the icon actually renders inside the static card (`CUBE_ANIM_GIF_PX = 70`), not a much bigger native size the README's own `<img width=...>` has to shrink.** The static card is a fixed `W=1500`px canvas displayed at `width="700"` (a `700/1500` ≈ 0.467× shrink), so a `scale=10`-upscaled icon that's natively `150px` (a 17px tile − 2px guide-strip = 15, ×10) renders on-screen at only `150 × 700/1500 = 70px` inside that card. `build_trigger_gif()` used to generate the gif at that same native `150px` and rely on the README's `width="70"` attribute alone to shrink it down to match — revised 2026-08-01 after the user spotted a real, visible size mismatch between DJ's `Speaker_Beat.gif` and `Speaker`'s own card icon, traced to a viewer (VS Code's markdown preview) not honoring `width=` the same way for an animated GIF as for a static PNG. Fixed by resizing every frame directly to `(CUBE_ANIM_GIF_PX, CUBE_ANIM_GIF_PX)` with `Image.NEAREST` before saving (no intermediate `scale` factor — the 15px source is a non-integer multiple of 70px either way, so one direct resize beats an arbitrary upscale followed by a second lossy shrink) — the stored file is now already the right size, so it displays correctly even in a viewer that ignores `width=` on GIFs. The README's `<img width="70">` stays (still correct, still worth keeping for viewers that DO honor it, e.g. GitHub), it just isn't load-bearing anymore.
- **A `CLOCK`-type `Animation:` line also gets a gif — `build_clock_gif()`, added 2026-08-02 — but state its cadence in the README caption, because the gif itself can't carry it.** A `CLOCK`'s thresholds are *fractions of one cooldown cycle*, and that cycle's real wall-clock length lives in the bound `Ability:` chain (45 seconds for General's `Rocket_Silo`, 0.7 for one of Unholy's `Two_Headed_Demon` bites) — not derivable from arbitrary DSL. So `build_clock_gif()` reproduces the cycle's *shape* (correct frame order, correct relative dwell per frame from `clock_frame_shares()`) compressed to a fixed `CLOCK_CYCLE_MS`, with a `CLOCK_MIN_FRAME_MS` floor so a 4%-of-cycle flourish frame doesn't flicker past and a `CLOCK_REST_MAX_MS` cap on the long rest frame a back-loaded threshold list produces. It does **not** claim real-time fidelity the way `TRIGGER`'s genuinely tick-derived durations do — which is exactly why every `CLOCK` entry in `sync_readme_preview.py`'s `ANIMATION_CAPTIONS` names the cadence outright ("Rocket Launch (every 45s)", "Left Head Bite (every 0.7s)"). `clock_frame_shares()` is the executable copy of the frame-share formula derived in `cube-chaos-scripting/references/cube-animation.md` — including the two edge cases a naive "share == thresholds[i]" reading gets wrong (the leading `t[0]` slice belongs to the LAST frame, and the last frame also absorbs everything past the thresholds' sum); read that section before touching it.
- **Look up a cube's companion animation gifs by its own parsed `Animation:` names, never by globbing `<Prefix>_Cubes_<CubeName>_*.gif`.** A glob is unsound the moment one cube's name is a prefix of another's, and it fails *silently* — it invents a plausible-looking animation for the wrong cube instead of erroring. Real incident, 2026-08-02: General has both `Rocket` and `Rocket_Silo`, so `General_Cubes_Rocket_*.gif` swallowed `General_Cubes_Rocket_Silo_Launch.gif` and gave `Rocket` a bogus animation named `Silo_Launch` (surfaced only because the missing-caption warning printed a mangled name — with a caption present it would have shipped a wrong README entry unnoticed). `sync_readme_preview.py`'s `cube_animation_gifs()` now takes the already-parsed `anim_types` map and checks `os.path.exists` per declared name, removing the ambiguity outright rather than trying to disambiguate filenames after the fact. **Any new code that needs "this cube's animation files" must do the same** — the same prefix-collision trap applies to any `<Prefix>_<Category>_<ItemName>_*` filename scheme, not just this one.
- **A `DOUBLE`-type `Animation:` line also gets a gif — `build_double_gif()`, added 2026-08-01 — but it's a fundamentally different rendering problem from `TRIGGER`, not just a second case of the same one.** `TRIGGER`'s frame comes from a self-contained countdown (ticks since `lastAbility` last fired) — fully computable with zero knowledge of the rest of the game. A `DOUBLE`'s frame comes from evaluating an arbitrary live "production" (`cube-chaos-scripting/references/cube-animation.md`'s Grammar section) — current hp, an ability's stacking count, whether a specific ability is currently held, a custom per-cube variable built up over a whole `EveryTick` state machine — none of which has a value outside an actual running battle. There is no deterministic playback to simulate, so this doesn't try to: `build_cube_animation_gifs()`'s default is `build_double_gif(mod_dir, cube_name, anim_name, list(range(amount)), out_path)` — every frame the sheet contains, cycled in raw sheet order at a flat `DOUBLE_FRAME_MS` (not derived from the `Animation:` line's own threshold list — those thresholds bucket a live VALUE into a frame index, they're magnitudes, not tick counts, so reusing them as gif timing the way `TRIGGER`'s genuinely-tick-based thresholds are would be meaningless). This default is honest and correct for most real cases (a plain wrapping counter, a small enumerable pose set, a set of skin variants picked once at spawn) since it never claims to know *when* a frame appears, only *that* it can. **When the raw sheet order genuinely doesn't read as one coherent loop** (frames for two visually distinct, real gameplay-recognizable states are interleaved in the same sheet — real case: Dinosaurs!'s `Red_Eye` interleaves a normal blink cycle with a below-half-hp "glare" variant across the same 10-frame sheet, traced by hand through its own `EveryTick` variable-computation logic, not guessed), split it into per-state gifs instead — one `build_double_gif()` call per state, each with its own hand-picked `frame_indices` subset and its own README table caption naming the STATE (`Above Half HP` / `Below Half HP`), not a trigger condition. See `ThirdParty/Dinosaurs/render_dinosaurs_preview.py`'s `DOUBLE_ANIMATION_STATES` for the worked example and its full derivation. No own-mod cube uses `DOUBLE` yet, so there's no override list in this shared script itself — add one here the same way (a small hand-maintained `(mod_prefix, cube_name, anim_name) -> [(label, [frame_indices]), ...]` dict) if/when an own-mod cube's raw order doesn't read cleanly either. See `cube-chaos-scripting/references/cube-animation.md` for the authoring-side implication of this (which animation TYPE to pick for a new cube, knowing how each one previews).
