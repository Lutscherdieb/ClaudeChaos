...
- **Shared footer row (both CUBE and PERK cards): class/species name (text only, no icon) on the left, referenced-cube icons (icon only, no name) to its right** — went through several rounds of user feedback while eyeballing rendered cards, each trimming it further:
  ...
  - **Referenced cubes, from TWO sources (`referenced_cubes_for()`), declared names first:**
    ...
    - A referenced name resolves to a real icon via the `CUBE_NAME_TO_ICON` global (`build_cube_icon_index()`, this mod's own `_Cubes.c.txt`/sheet, computed once per mod in `render_mod()` and shared by every card category — a `PERK:` card resolves against the same pool as a `CUBE:` card, not a separate lookup). **A name with no resolvable icon is dropped entirely, not kept as a name-less blank entry** — since this footer never shows text, an icon-less entry contributed zero visible content but still reserved a full footer row's worth of blank vertical space (caught comparing Great_Wall's `Great_Wall` terrain-perk card before/after this fix — its `ReferenceCube: Anchored_Basalt`/`Water`/`Catapult` name terrain-scenario `TOKEN` cubes that live outside any `_Cubes.c.txt`, so none resolve to an icon, and the card had grown a blank gap at the bottom for nothing).
