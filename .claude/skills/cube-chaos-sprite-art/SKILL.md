...
- **CUBE cards (only) use a dedicated `render_cube_card()` layout, not the shared `render_card()` used for every `PERK:` category** (added 2026-07-31, after the user supplied a real gameplay screenshot of Unholy's `Brimstone` tooltip). Differences, each traced to something the screenshot actually showed:
  - **A boxed mana value + a red HP bar** in a left stat column, instead of a plain `Mana Cost: X | HP: Y/Z` text line. HP always renders `current/max` even when equal (e.g. `6/6`, not collapsed to `6`) — confirmed from the screenshot, which showed `6/6` for a full-health cube.
  ...
  - **A single shared footer row: class/species name+icon on the left, referenced-cube icon+name pairs to its right, no category label on either half** — NOT two stacked blocks. An earlier version stacked a whole separate "Referenced Cubes" block (its own label line + a row of icon-over-name entries) underneath the class/species line; user feedback 2026-07-31 was that this grew the card's height too much whenever a cube had any referenced cubes at all, so both are now laid out on one shared row instead (`footer_row_h` sized to whichever of the two is taller — the class/species icon, or the tallest referenced-cube icon/a bare text line — and every element in the row vertically centered against that one height). Referenced-cube entries are icon-then-name inline (not icon-above-name) specifically so the row stays exactly as tall as a single line of content, matching the class/species entry's own icon-then-name shape. A follow-up round of the same feedback also dropped the "Referenced Cubes:" text label itself (further trimmed once shown next to the class/species entry) — the icon+name pairs alone are self-explanatory in context, immediately to the right of the class/species entry.
  - **Referenced cubes**: any cube this cube's own `Ability:` chains create/copy via a `CubeConstant <Name>`/`HiddenCubeConstant <Name>` token pair (`find_referenced_cubes` — a `CUBE`-typed arg is always spelled as that keyword plus the name, never the name alone, same fact `resolve_builtin_ability_text` already relies on). Scans the WHOLE block body, not just top-level lines, since these references usually live on indented sub-lines of a chain. **Also expands one hop into a bare top-level `Ability: <CompoundName>` grant's own body** — DJ's `Speaker` grants `Ability: SpeakerNoteSpawn` with zero inline args, and it's `SpeakerNoteSpawn`'s own body (not Speaker's) that contains `CubeConstant Note`; scanning only the cube's own block missed this entirely, caught reading the rendered card back (no "Note" in Speaker's row despite it visibly spawning one). Not followed recursively past one hop — no compound-granting-a-compound case exists in this repo's mods yet. A referenced name found in this mod's own `_Cubes.c.txt`/sheet gets its real icon; a name from the base game or another mod (none exist in this repo's mods yet) falls back to name-only text rather than crashing or being silently dropped.
  - **Class/species name+icon**, in that class/species's own color. `find_class_species()` reads slot 0 of whichever file `perks_source_basenames()` finds first — confirmed across every mod that has one (DJ/General/Broker's class perk, Unholy/Voidling's species perk) that this is always the file's first `PERK:` block, matching the "Base class/species icon style" fill-equals-border convention above. **Sample the color with `sample_dominant_color()`, which excludes the SHEET's own default background `(0,148,255)`, not this script's black card-canvas `BG`** — an early version of this popped the wrong background color and returned the sheet's blue as "the class color" for Unholy (verified: a guaranteed-blank/mostly-background tile is ~72% `(0,148,255)` pixels vs ~28% the real `(150,20,20)` icon/border color, so the wrong-background version reliably picks the wrong color, not just occasionally). Mods with no class/species of their own (`Great_Wall`, `Home_Turf_Advantage` — Terrain/Neutral-only) simply omit that half of the row; in practice this never comes up anyway since neither of those mods has a `_Cubes.c.txt` to begin with.
  - **A faint steady grey outline around the icon** (not the game's own sprite border — a decoration this script itself draws, framing the icon like the in-game library screen's un-pinned card-slot outline the user pointed at, minus that screen's thicker corner-square accents which are an unrelated selection-state indicator). Gated behind the gitignored `.claude/preferences.local.md`'s `preview_card_icon_border: on|off` (see `cube-chaos-repo-setup`), default `on` — flip and regenerate to compare, no code change needed.
