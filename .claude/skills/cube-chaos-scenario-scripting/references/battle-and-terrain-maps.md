...
**A terrain's battlefield can be rendered straight from its own map data, no game launch needed** — `.claude/skills/cube-chaos-scenario-scripting/scripts/render_terrain_screenshot.py` parses a terrain's `ADDITIONALMAP:` blocks into a grid and draws each cell using the real cube's own icon (resolved the same way `render_preview_cards.py`'s Referenced Cubes row does — this mod's own `_Cubes.c.txt` first, then `base_game_cube_icon_index()`'s base-game fallback), tiled at 30px/cell against a sky-gradient background sampled from a real screenshot (52x28 tiles → 1560x840px, matching real screenshots' own dimensions exactly). Scope: only the terrain's own ground-layout partial(s) — the shared `Battle_*_Player`/`Battle_*_Enemy` leader partials place an engine-hardcoded `Empty`/`Difficulty_Leader` placeholder with no real sprite (the actual Leader art is spawned by the engine at battle start, not present in any map DSL), so a generated image omits leader/HUD icons a real screenshot has. Edit the hardcoded output list at the bottom of the script to point at a different terrain's `_Maps.c.txt`/scenario names.
