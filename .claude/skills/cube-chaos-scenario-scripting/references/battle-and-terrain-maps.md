...
- **`PLACERECT: x y width height faction`** — marks a rectangular zone as belonging to a faction for cube-placement purposes during the battle. Every terrain scenario ends with 2 (player half + enemy half) or 3 `PLACERECT:` lines — a third one at faction `3` marks a thin neutral middle strip for terrains with a raised second lane (`Mountains`, `Hovering_Vault`, `Loop_Monument` all do this; most terrains just split the map into two halves). **The boundary between two adjacent `PLACERECT:` zones is drawn in-game as a thin (1px-at-30px/tile) vertical highlight line**, visible only through open sky/gaps — it's occluded by any solid terrain drawn on top, confirming it's a background-layer decoration rather than a foreground UI overlay. Confirmed by sampling a real screenshot (`GameData/Great_Wall/Screenshots/GreatWall.png`) at the zone boundary (x=26 tiles): a consistent ~16.4% white alpha-blend over whatever sky color would otherwise be there at every row checked (delta scales with the base color, not a fixed additive offset — solving `base*(1-a)+255*a = observed` gives the same `a≈0.164` at multiple y rows). `render_terrain_screenshot.py`'s `placement_zone_boundaries()`/`DIVIDER_BLEND` reproduce this: draw the blended line into the sky layer first, then paste opaque cube tiles on top so they naturally occlude it.
- **`CAMPAIGNSETUP: X Y`** — appears only inside the shared `Battle_<Height>_Player` partial scenarios (never `_Enemy`, never the terrain-ground ones). **Takes two literal tile coordinates, not `faction x`** (corrected 2026-08-01, decompiled `ScenarioReader`'s `"CAMPAIGNSETUP:"` case: `int X = T.NextInt(); int Y = T.NextInt();` then `Game.Campaign.GetLeader().CreateInWorld(World.WorldGrid[X][Y], ...)`) — it directly places the PLAYER's own campaign leader cube (whichever class they picked, not a fixed sprite) at that exact tile. `Battle_Normal_Player`'s own `CAMPAIGNSETUP: 1 21` places the leader at x=1 (one tile in from the map's left edge), y=21 (the Normal tier's leader row, same row `Battle_Normal_Player`'s own `DATA: 1 0 21 2`/`DATA: 1 1 20 2` reservation cells already established). The enemy side has no `CAMPAIGNSETUP:` equivalent — `Battle_<Tier>_Enemy` instead places a real, name-resolvable `Difficulty_Leader` token cube via an ordinary `DATA:` line (e.g. `Battle_Normal_Enemy`'s `DATA: 1 51 21 2`), and any OTHER enemy units come from that battle's own `RANDOMFITTINGSETUP:`-driven AI hand, not fixed map data at all.
...
**A boss battle's map is the terrain's normal ground partial PLUS its boss-ground partial, both composited — not the boss partial alone.** `MapGenerationTPEB`'s `COMPOUND: ABILITY` body (above) has exactly one `GenericAction` placeholder per signal branch, in the fixed order ground(1)/player(2)/enemy(3)/boss-ground(4), and a terrain's own `WorldAbility: MapGenerationTPEB` fills those 4 slots positionally with its 4 `ReadAPartialScenario` calls. A boss battle's `START_ACTION:` only fires 3 of the 4 signals — `Battle_Terrain_Generation`, `Battle_Boss_Generation`, `Battle_Player_Generation`, never `Battle_Enemy_Generation` — which resolves to slots #1 (ground) and #4 (boss-ground) BOTH running, slot #3 (enemy) never firing. Confirmed by reconstructing `Great_Wall`'s own boss map purely from data (`GameData/Great_Wall/Great_Wall_Maps.c.txt`, via `.claude/skills/cube-chaos-scenario-scripting/scripts/render_terrain_screenshot.py`) and comparing against a real in-game screenshot: `Great_Wall_Boss_Terrain`'s own `DATARECT:` data places only ONE wall (at x=37), but the real screenshot shows TWO — the first wall only appears because `Great_Wall_Terrain` (the normal ground partial) is composited in underneath it. When building a new terrain's boss-ground partial, remember its content LAYERS ON TOP of the normal ground partial rather than replacing it — author boss-only additions (a second wall, extra hazards) rather than re-declaring the whole floor, and don't be surprised that a `<Name>_Boss_Terrain` file with sparse `DATARECT:`/`DATA:` content still produces a full battlefield.

**A terrain's battlefield can be rendered straight from its own map data, no game launch needed** — `.claude/skills/cube-chaos-scenario-scripting/scripts/render_terrain_screenshot.py` parses a terrain's `ADDITIONALMAP:` blocks into a grid and draws each cell using the real cube's own icon (resolved the same way `render_preview_cards.py`'s Referenced Cubes row does — the terrain's own `<ModPrefix>_Cubes.c.txt` first via the `mod_dir`/`mod_prefix` arguments, then `_all_base_game_blocks()`'s base-game fallback), tiled at 30px/cell against a sky-gradient background sampled from a real screenshot (52x28 tiles → 1560x840px, matching real screenshots' own dimensions exactly). Scope: only the terrain's own ground-layout partial(s) — the shared `Battle_*_Player`/`Battle_*_Enemy` leader partials place an engine-hardcoded `Empty`/`Difficulty_Leader` placeholder with no real sprite (the actual Leader art is spawned by the engine at battle start, not present in any map DSL), so a generated image omits leader/HUD icons a real screenshot has. **It also renders the map exactly as authored and does not simulate gravity or evaluate any placed cube's abilities** — so an unsupported feature that will collapse at battle start, and a faction-`0` copy of an ability-bearing cube that will sit inert, both look perfectly correct in the PNG. Those two are the checks it cannot do for you; see the two rules below.

**This is wired into `content-terrain.md`'s own workflow, not just a standalone debugging tool** — generate and show this screenshot to the user right after writing a new terrain's ground-layout partial(s), as a fast layout-verification loop before spending a full test-launch on it. `render_terrain_mod()` is a reusable function (map file, output dir, an `(filename, [scenario names])` outputs list, optional `mod_dir`/`mod_prefix`), called once per terrain mod from a "Registered terrain mods" call list at the bottom of the script — the same "hardcoded list, extend when a new mod needs it" convention `render_preview_cards.py`'s own `render_mod(...)` calls already use (see `cube-chaos-mod-setup` SKILL.md). Add a new call there for a new terrain mod rather than repointing `GREAT_WALL_OUTPUTS`. `.claude/hooks/regen-terrain-screenshots.sh` (a PostToolUse hook, universal `GameData/*.c.txt` scope like `regen-preview-cards.sh`) reruns every registered mod's screenshots automatically after any content edit — but only mods actually registered in the script get regenerated, so registering a new terrain mod is still a required manual step.
...
## Give every ability-bearing terrain cube to the faction meant to USE it — faction `0` disables any ally-gated ability

**Rule: when a terrain places a cube whose ability is gated on `IsAllyToCaster`/`IsEnemyToCaster` or uses `Forwards`/`Backwards`, set that `DATA:`/`DATARECT:` line's faction argument to the faction meant to benefit (`1` player side, `2` enemy side) — never `0`.** Faction `0` is correct only for inert decoration (a rock floor, an ocean, a wall with no ability). A neutral cube has no allies for `IsAllyToCaster` to match and no meaningful facing for `Forwards`/`Backwards` to resolve against, so its ability silently does nothing and it degrades into a plain wall — with no log warning, since nothing failed to parse.

**Verify it before launching:** for every cube name a terrain places, grep its `CUBE:` block for `IsAllyToCaster`/`IsEnemyToCaster`/`Forwards`/`Backwards` (`grep -A15 "^CUBE: <Name> " <file>`). Any hit means that cube's placing line must carry faction `1` or `2`, not `0`. A terrain that reads correctly in `render_terrain_screenshot.py` proves nothing here — the screenshot draws the tile identically at any faction (bar the mirror flip), so this is exactly the class of bug a picture can't catch.

**Evidence.** `Staircase` (`Main/3GeneralCubes.c.txt:5959`, a real obtainable `IDENT 1` cube, `TYPE Wall` + `TYPE Movement`) is the load-bearing case: its whole point is `Ability: BeforeACubeMoves If IsAllyToCaster Victim ... TargetCube Victim MoveInDirection North` — "before an ally moves into this horizontally move it upwards (they can climb over this)", i.e. it is the engine's stock **traversal** primitive for a terrain with height. The base game places it exactly once in the entire map layer (`Extra_Mechanics/Battle_Maps.c.txt:192`, `Mountain_Boss_Terrain`), as three steps at **faction 2** (`DATA: 8 44 10 2`, `8 45 11 2`, `8 46 12 2`) climbing to that terrain's own boss — deliberately the boss's own stair, not neutral scenery. Applied in `GameData/Crusader/Crusader_Maps.c.txt` (`Stairway_To_Heaven_Terrain`, 2026-08-03): the player's 18-step stair is faction `1`, the mirrored enemy stair faction `2`, and only the inert pieces (cloud beds, the platform's middle tiles, the summit's `Mana_Crystal`s) are faction `0`. This also makes each side's stair destructible by the other, which is a feature — denying an opponent their climb becomes real play.

**Faction-symmetry note:** giving each side its own mirrored copy is what keeps such a terrain symmetric — it is the scenario-layer expression of the same mandate `CLAUDE.md` states for `Ability:` chains (see `cube-chaos-scripting/references/targeting-movement-board.md`). One shared neutral structure is *not* the symmetric option here; it's the broken one.

## A floating terrain feature needs `Flying` under every tile — `Burrowed` is NOT anti-gravity

**Rule: any terrain tile not resting on a stack that reaches the map's bottom row must be a cube with `Ability: Flying`, or sit directly on top of one.** Gravity applies to map-placed cubes exactly as it does to units, so an unsupported decoration falls at battle start.

**`Burrowed` looks like the anti-gravity ability and is not.** Its `COMPOUND: ABILITY` body is literally `Nothing` (`Base_Core/1Compounds.c.txt`) and `ModdingInfo.txt:90` defines it as *"Won't be attacked by `Melee` from above"* — a melee-immunity marker with zero movement effect. The real anti-gravity ability is **`Flying`**: `BeforeThisMovesNegation If IsSameCube Culprit WorldCube NegateX DoubleConstant 1`, i.e. it negates specifically the gravity `WorldCube`'s pull (see `cube-chaos-scripting/references/targeting-movement-board.md`'s gravity section for the full negation idiom and its `Hovering`/`Buoyant` variants). `Earth`/`Rock`/`Granite`/`Basalt`/`Anchored_Basalt` carry `Burrowed` and never fall **only because every real ground fill is stacked up from row 27** — their support is the map floor, not the ability. Copying `Burrowed` onto a mid-air tile therefore looks precedented and silently fails.

**The base game's own floating-terrain cube is `Floating_Tungsten`** (`Extra_Mechanics/TokenCubes.c.txt:618`) — exactly `Burrowed` + `Flying`, and what `Loop_Monument_Terrain` uses for every platform suspended in open air. Reach for it, or copy its two-ability shape, rather than inventing a floating tile from `Burrowed` alone.

**Verify it before launching:** for every placed tile at `(x, y)`, confirm either `(x, y+1)` is filled by some line in the same partial, or that tile's own cube has `Flying` — walk the coordinate list in code, don't eyeball the screenshot. `render_terrain_screenshot.py` renders the map **as authored** and simulates neither gravity nor any placed cube's abilities, so a terrain that will collapse on the first battle looks perfectly intact in the generated PNG. Together with the faction rule above, these are the two checks that image cannot do for you.

**Evidence / incident (2026-08-03).** Crusader's `Stairway_To_Heaven_Terrain` shipped its 36 stair beds and summit platform on a `Cloudstone` TOKEN carrying `Burrowed` only, on the strength of an earlier, wrong version of this very section. It parsed clean, logged nothing, and rendered correctly — the whole stairway would have fallen on the first battle. Caught only when the user independently asked about `Flying`; fixed by adding `Ability: Flying` to `Cloudstone`, matching `Floating_Tungsten`. Note also that the base game's own `Mountain_Boss_Terrain` staircases at `(44,10)`/`(45,11)`/`(46,12)` have nothing beneath them and no `Flying` either — don't copy that placement as a worked example.

## Wrapping a column's bottom back to its top (`Loop_Chasm`)

**To make falling loop instead of ending on the floor, place `Loop_Chasm` on the bottom row (`y=27`) of that column and leave the column above it OPEN.** `Loop_Chasm` (`Extra_Mechanics/TokenCubes.c.txt:606`, `TOKEN`, 15/40/40, faction `0` in every real placement) rewrites adjacency once per second so that the tile above it leads to `(thisX, 0)` and back:
```
Ability: EverySecond Both ChangeIncomingAdjacency PositionOfThis
 South PositionFromXAndY XOfPosition PositionOfThis DoubleConstant 0
 NewAdjacency PositionFromXAndY XOfPosition PositionOfThis DoubleConstant 0
 North PositionOfThis
Ability: Quantum
Ability: Unmovable
```
It only affects **its own column**, so looping N columns needs N of them — a `DATARECT:` across the bottom row is the idiom. `Quantum` ("after this dies, teleports or moves reset all adjacency at the previous position") is what keeps the rewritten adjacency from leaking once something moves through, so grant it alongside rather than using `ChangeIncomingAdjacency` bare.

**A looping column MUST contain something solid somewhere, or anything entering it falls forever.** `Loop_Monument_Terrain` satisfies this by looping only a few deliberate chasms while keeping a real floor elsewhere. **Verify with code, not by eye:** build a `column -> solid rows` map from the partial's own `DATA:`/`DATARECT:` lines and assert no column is empty. This is cheap and catches the one failure mode that is genuinely unrecoverable in play.

**Budget for how the loop LOOKS before committing to it, not just how it behaves — `Loop_Chasm`'s own icon is a bright yellow-flecked portal frame around a black void, and one per column means that art tiles across the entire bottom row.** Real outcome (Crusader `Stairway_To_Heaven`, 2026-08-03): a full 52-column loop was built, worked correctly, and was then removed purely because a row of portals read as wrong on a serene sky map — the mechanic was fine, the visual dominance wasn't. If a loop is wanted on a map with a strong art direction, either confine it to a few deliberate chasms the way `Loop_Monument` does, or define a mod-own `TOKEN` carrying the same three abilities (`ChangeIncomingAdjacency`+`NewAdjacency` body, `Quantum`, `Unmovable`) with art that matches the terrain — base-game sprites can't be restyled (`CLAUDE.md`), so a mod-own twin is the only way to keep the mechanic and lose the look.
