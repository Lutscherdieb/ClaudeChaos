...
- **Upgrade-file separation**: grep each mod `.c.txt` for `IsUpgradeFrom:` and confirm the file it's in has no matching `Sprites/*.c.png` (i.e. it's a dedicated `_UpgradePerks.c.txt`, not mixed into a sprited regular-perks file).
...
- **Sheet math check**: for each `.c.png`, confirm `width == height` and `width / tile_size == ceil(sqrt(block count in the matching .txt))`; confirm the basename matches its `.txt` file and doesn't collide with any already-loaded base-game or other-mod package name.
