...
- **Hardcoded faction/edge**: grep the mod for `PlacabilityOfPosition`. Every hit should be compared (via `IsEqual`/`Not IsEqual`) against `FactionOfThis` or `FactionOfCube <someone>` — a comparison against a literal `DoubleConstant 1`/`DoubleConstant 2` instead is a candidate finding (the ability only behaves correctly for one player). **Known legitimate exception, don't report it:** a `ClickAction:`-driven consumable is player-only by construction, so a literal `DoubleConstant 1` there is correct — real base-game precedent, `Main/Consumables.c.txt:103` (`Bottled_Safety`) and `Main/UpgradeConsumables.c.txt:95` (`Reusable_Bottled_Safety`). Check whether the enclosing block is player-triggered before flagging. Separately, grep for `PositionFromXAndY DoubleConstant 0` and `MaxWorldX` inside `Ability:`/`WorldAbility:` chains: an "edge of the board" used as *my own* edge must be picked by testing placability, not assumed — a chain that always uses `0` (or always `MaxWorldX`) without branching on faction fires at the wrong edge for the opponent. Both are the same underlying rule as the East/West check above, one level up (whole-board orientation rather than one step sideways). **A hit under `WorldAbility:` rather than `Ability:` is a bigger finding than the one this row describes** — per `cube-chaos-scripting/references/targeting-movement-board.md`'s closing note, a territory check in a `WorldAbility:` block can parse clean and silently do nothing at runtime, so report that as the primary issue rather than just the faction literal.
- **Placement-preview coverage (Universal — always check)**: for each `CUBE:` block, grep it for `PositionInDirectionFrom` (the bare prefix — it must catch **both** `...FromPosition <Dir> PositionOfThis` and the shorter `...FromThis <Dir>`), `CubeInDirectionFromCube` and `TopPositionAboveCube`. Classify each hit against `cube-chaos-scripting/references/authoring-and-inheritance.md`'s required-vs-omit table, then compare the required-offset count against the block's actual `Visual:` line count. **Both directions are findings**: too few (a fixed-offset effect with no marker — the `Hell_Portal` case) and stale/wrong (a marker whose offset doesn't match the ability — the `Keyboard` case below).

  **Run it in BOTH directions, because each direction has a different entry condition and neither finds the other's bugs:**

  | Direction | Entry condition | Finds |
  |---|---|---|
  | too few | block HAS a positional token, but fewer `Visual:` lines than offsets | a marker never written |
  | **wrong / shouldn't exist** | block HAS `Visual:` lines but **no** fixed-offset token anywhere | a marker inherited by copy-paste |

  **Expand one hop into any bare `Ability: <CompoundName>` before deciding either way.** A block's positional token frequently lives in a `COMPOUND: ABILITY` body, not the block itself — scanning only the block's own lines makes such a cube invisible to *both* directions. (`render_preview_cards.py`'s `referenced_cubes_for()` does the same one-hop expansion for the same reason.)

  **Known legitimate exclusions, don't report them:** a destination of `PositionOfThis`/`OriginalPosition` (own tile — offset `0 0` is never used in the base game); a random/dynamic destination (`ARandomPositionWhich`, above `ARandomEnemy`, a `Victim`'s position); an effect granted onto another cube via `Enchantment`/`GainAbilityText`; `TopPositionAboveCube Caster` paired with `Visual: Target 0 -1` + a bare `Visual: Infinite` (the documented own-column idiom — `General`'s `Bombardement`); and a `Visual: Sword`/`Arrow` marker belonging to a melee/ranged ability (`EveryXMeleeY` etc.) rather than to a spawn (`Unholy`'s `Cultist`). Note `TOKEN` cubes are in scope, not exempt — 65 base-game `TOKEN`s carry `Visual:` lines.

  **Real incident, 2026-08-03 — why the second direction exists.** DJ's `Keyboard` carried `Visual: Square 1 0 96 96 96` (the forward tile) while `KeyboardNoteSpawn` actually spawns at `TopPositionAboveCube Storage` where `Storage` is `ARandomEnemy` — a dynamic destination on the *enemy's* column, which the table says gets **no marker at all**. The line was inherited verbatim from `Speaker`, where the identical line is correct. It was user-spotted in play ("why does keyboard have a front facing visual when placed, it does not create the note there?"), having survived both the 2026-08-02 placement-preview sweep (scoped to *missing* markers only) and `.claude/hooks/check-visual-coverage.sh`, which by design **only fires on blocks with zero `Visual:` lines** — its comment says a block that already has one "is assumed to have been thought about." **A copy-pasted block defeats that assumption exactly**: it arrives pre-populated with someone else's correct marker. So the hook cannot cover this class and this recipe is its only backstop.

  **Corollary rule for authoring, not just auditing: when a new `CUBE:` block is derived by copying an existing one, re-derive every inherited `Visual:` line from the new block's own ability chain — never carry them over.** Same discipline as re-deriving the `Text:`; see `cube-chaos-scripting/references/authoring-and-inheritance.md`'s `Visual:` section.
