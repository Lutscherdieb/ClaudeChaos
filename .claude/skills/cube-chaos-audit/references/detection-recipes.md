...
- **Placement-preview coverage (Universal — always check)**: for each `CUBE:` block, grep it for `PositionInDirectionFromPosition`, `CubeInDirectionFromCube` and `TopPositionAboveCube`. Every block with at least one hit is a candidate; classify each hit against `cube-chaos-scripting/references/authoring-and-inheritance.md`'s required-vs-omit table, then compare the required-offset count against the block's actual `Visual:` line count. **Both directions are findings**: too few (a fixed-offset effect with no marker — the `Hell_Portal` case) and stale/wrong (a marker whose offset no longer matches the ability). **Known legitimate exclusions, don't report them:** a destination of `PositionOfThis`/`OriginalPosition` (own tile — offset `0 0` is never used in the base game), a random/dynamic destination (`ARandomPositionWhich`, above `ARandomEnemy`, a `Victim`'s position), and an effect granted onto another cube via `Enchantment`/`GainAbilityText`. Note `TOKEN` cubes are in scope, not exempt — 65 base-game `TOKEN`s carry `Visual:` lines. The same non-blocking check runs on Stop via `.claude/hooks/check-visual-coverage.sh`.
