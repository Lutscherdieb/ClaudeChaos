...
| No hardcoded faction number or board edge where a faction-relative check was meant — a territory/side test compares `PlacabilityOfPosition` against `FactionOfThis`/`FactionOfCube Caster`, not `DoubleConstant 1`/`2`, and an "edge of the board" derives from that comparison rather than assuming `0`/`MaxWorldX` is always ours | `references/targeting-movement-board.md` — "Enemy/allied territory..."; see also `CLAUDE.md`'s faction-symmetry rule | Bug | Universal |
| Every `CUBE:` with a fixed-offset positional effect (spawns, damages, buffs or heals a specific tile relative to itself) carries a matching `Visual:` placement-preview line per affected offset — and cubes whose target is their own tile or a random/dynamic position deliberately carry none | `references/authoring-and-inheritance.md` — "`Visual:` — placement-preview markers" (the required-vs-omit table) | Bug | Universal |
...
| New `CUBE:`/`PERK:` blocks were appended at file end, not inserted mid-file (sprite slot order intact) | `SKILL.md` — "Block formats"; also `cube-chaos-sprite-art`'s "Adding a new `CUBE:`/`PERK:` — the append procedure" | Bug | Universal |
| A tool/script reading a sprite sheet derives its column count from the sheet's real pixel width, not `ceil(sqrt(block_count))` — the two silently disagree once a hand-edited sheet lags its `.c.txt` block count, misaligning the whole sheet | `cube-chaos-sprite-art` — "Fixed tile sizes" (the `real_cols` corollary) | Bug | Universal |
...
| A `BelongsTo: CLASS`/`SPECIES` perk with a `CampaignAbility: AfterAMapIsGenerated` chain also fires that chain from an `ObtainAction:` (otherwise it silently does nothing on the first map, generated before class selection) — **via a shared `COMPOUND: ACTION`, not two hand-copied chains**, which drift the moment one side is edited | `cube-chaos-scenario-scripting/references/nodemap-generation.md` — "The first map is generated BEFORE class/species selection" | Bug | Universal |
| A `MapNodeIsType ... Battle` filter meant for *normal* battles carries the `Not MapNodeIsType ... Boss` exclusion — the tag matches cursed and boss battles too | `cube-chaos-scenario-scripting/references/nodemap-generation.md` — "`MapNodeIsType` is a **tag** test, not scenario-name equality" | Bug | Universal |
| An ability's duplicate-grant model matches its shape: `NO_DUPLICATES`+`STACKING` for a persistent passive, `NO_DUPLICATES` alone for a marker tag, and **neither** for a self-removing one-shot (which needs independent per-instance timers) | `cube-chaos-scripting/references/authoring-and-inheritance.md` — "Duplicate-grant model: pick it from the ability's SHAPE" | Bug | Universal |
| A `Before*` ability that moves/damages/heals a cube **other than its own owner** either guards the triggering action's precondition (destination `PositionExists`+`IsPositionEmpty`) or uses the outcome-safe `After*`+`OriginalPosition` shape — `Before*` fires on the attempt even when it fails | `references/targeting-movement-board.md` — "A `Before*` trigger fires on the ATTEMPT, not the outcome" | Bug | Universal |
| A self-propagating granted ability (one that re-grants itself via `GainThisAbility`) has an `AmountOfAbilitiesOfCubeWithName` concurrency cap — without one the instance count multiplies by the neighbour count each generation | `cube-chaos-scripting/references/authoring-and-inheritance.md` — same section; branching-process reasoning in `references/death-fusion-reactive.md` | Bug | Universal |
