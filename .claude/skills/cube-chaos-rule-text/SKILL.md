...
Colour-only also keeps our descriptions short: the player can hover the ability on the cube itself for the detail. Grab the exact colour and display name from the ability's own entry in `ModdingInfo.txt` rather than guessing — real values include `StrengthX` `255 38 0` "Strength N", `GrowthX` `0 127 14` "Growth N", `ChargeEveryX` `155 238 255` "Charging", `Swarm` `255 0 220` "Swarm", `Flying` `109 209 228` "Flying", `Burning` `255 106 0` "Burning" (`ModdingInfo.txt:89`, a 0-arg keyword — write `\C255 106 0 Burning \CN`, no number). **Don't append a hand-written parenthetical explaining a base keyword** — that's the duplication this rule exists to avoid.
