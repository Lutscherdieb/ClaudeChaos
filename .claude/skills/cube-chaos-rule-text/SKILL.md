...
**But name the spawned cube only — do NOT restate its own abilities in the spawner's text.** The game already shows the spawned cube's rules in-game (when a cube spawns another, the created cube's own ability list is viewable), so listing them in the parent's `Text:` is redundant. The base game's own `Ritual` says "create an allied Imp above" — not "create an allied charging acidic hovering Imp." Real correction, the Unholy mod: `Brimstone`'s text first read "create a neutral \C255 106 0 Burning \CN \C155 55 134 Acidic \CN Molten_Brimstone..." and the user cut the keyword restatement down to just "create a neutral Molten_Brimstone..." because `Molten_Brimstone` carries `Burning`/`Acidic` by default and the game surfaces them. So: name + allegiance (per above) + *what the spawn does that isn't already on its own card* (positioning like "at the top of its column, which falls from there" is fair game since that's the spawner's placement choice, not the cube's intrinsic ability) — but never re-enumerate the spawned cube's baked-in abilities.
