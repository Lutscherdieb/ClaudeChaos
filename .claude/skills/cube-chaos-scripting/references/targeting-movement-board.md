...
## `AfterThisCollides` is NOT the same binding as `AfterACubeCollides` — the collided-into cube is `Target`, not `Storage`

**Real bug, the General mod's `Medic` (2026-07-31):** `Ability: AfterThisCollides If IsAllyToCaster Storage TargetCube Storage HealXDamage DoubleConstant 1` parsed clean and worked often enough in practice to ship, but silently failed to heal the cube it moved into when the Medic was enemy-AI-controlled (user-reported: "does not heal the enemies it moves into"). Root cause confirmed by decompiling `Cube.class`'s movement-collision method (CFR against `Cube Chaos.jar`, see [[reference_jar_decompile_technique]]): the engine's collision handler sets **`EventClone.Target = Into`** for `this.AfterThisCollidesTriggerQ` (the cube's own self-trigger — `Into` is the other cube in the collision), but only sets `EventClone.Storage = Into` for the separate, global `this.World.AfterCollisionTriggerQ` (the trigger that backs `AfterACubeCollides`, documented in the section above). `CUBETarget.Value(E)` returns `E.Target`; `CUBEStorage.Value(E)` returns `E.Storage` — two genuinely different `Event` fields, confirmed by decompiling both `dw/game/dd/Code/CUBE/CUBETarget.class` and `CUBEStorage.class`. So on `AfterThisCollides` specifically, `Storage` is never populated by the trigger at all — reading it returns whatever stale value the cube's `Storage` slot last held from an unrelated `SetStorage` call (or nothing), which is exactly the kind of undefined-behavior-dependent bug that can look like it "mostly works" while actually being wrong. **Fix: use `Target`, not `Storage`, for the other cube on `AfterThisCollides`** — `Ability: AfterThisCollides If IsAllyToCaster Target TargetCube Target HealXDamage DoubleConstant 1`, confirmed clean on a fresh launch after the change. `AfterThisCollides` has exactly one real usage in the entire base game + all mods (this `Medic` cube), so there was no existing precedent to check against before this fix — audited every other `Storage`/`Target` usage across `DJ`/`General`/`Unholy`/`Voidling`/`Broker`/`DJ_Voidling` afterward and found no second occurrence of this specific mistake (every other `Storage` read is preceded by its own `SetStorage` in the same ability, and the one other collision-reactive ability in this repo, `DJ`'s `Echo` perk, correctly uses `AfterACubeCollides` + `Storage`, not `AfterThisCollides`).
