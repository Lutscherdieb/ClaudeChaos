...
**The same rule has a scenario-layer counterpart worth knowing before authoring a battle map:** a cube written correctly with `Forwards`/`IsAllyToCaster` still does nothing if a `DATA:`/`DATARECT:` line places it at faction `0`, since a neutral cube has no allies and no facing. See `cube-chaos-scenario-scripting/references/battle-and-terrain-maps.md`'s "Give every ability-bearing terrain cube to the faction meant to USE it".
...
**Gravity applies to map-placed terrain cubes too, which is a live authoring trap when writing a battle map rather than an ability** — a diagonal or floating terrain feature collapses at battle start unless every tile sits on something `Burrowed`, and the terrain screenshot renderer doesn't simulate it. See `cube-chaos-scenario-scripting/references/battle-and-terrain-maps.md`'s "A diagonal/floating terrain feature needs a `Burrowed` bed" for the rule and its verification step.
