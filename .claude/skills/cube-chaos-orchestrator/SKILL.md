...
**`DESIGN.md` holds rationale, not hard numeric facts.** Don't restate a cube/perk's exact mana cost, hp, or ability-parameter numbers (timers, damage amounts, thresholds) in `DESIGN.md` prose just because it's convenient to read them there — those live in the mod's own `.c.txt` as the actual source of truth, and a copied-in number has no reason to get updated when the real one is rebalanced, so it silently drifts. **Real incident (2026-07-25): General's ammo-cube mana-cost list drifted from the real value to a stale one, got "fixed," and drifted right back** because the fix landed in a memory file, not in `DESIGN.md` itself, and nothing forced the two to stay in sync. When a number matters to the point being made, point at the file instead of restating it — `"see General_Cubes.c.txt for current stats"` — and keep only the reasoning that isn't otherwise recoverable (why a choice was made, a pricing anchor/formula, a deliberate tradeoff). A number that's stable because it's an immutable base-game constant we never edit (e.g. a shared compound's hardcoded timing) is a legitimate exception and safe to cite directly.
