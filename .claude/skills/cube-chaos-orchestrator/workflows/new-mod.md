...
6. Create a minimal `GameData/<Mod>/README.md` skeleton: mod name, a one-line placeholder description (to be filled in once real content exists), an empty `## Preview` heading, and the "regenerate the preview cards" section with its `---` monorepo-only-note callout (see `cube-chaos-mod-setup`'s "Each mod keeps its own README.md..." section and `GameData/DJ/README.md` for the exact shape to copy) — get the mod-content-vs-monorepo-tooling distinction right from the start rather than retrofitting it later. Don't try to generate preview images yet — there's no content or sprites to render (the preview-card script gets its first real run once the first content workflow adds a cube/perk). Also add a link to it from the repo root `README.md`'s mod list, matching how the DJ mod is linked there.
