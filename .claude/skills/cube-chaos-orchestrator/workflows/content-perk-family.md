...
**Every category in the table above (Curse, Blight, Boon, Consumable, Golden, Neutral, CubeUpgrade) gets its own `<ModPrefix>_<Category>.c.txt` file with its own matching `Sprites/<ModPrefix>_<Category>.c.png` — never appended into `<ModPrefix>_Perks.c.txt`, which is reserved for plain `BelongsTo: <ClassName>/<SpeciesName>` reward perks.** Decide this before writing the `PERK:` block, not after — see `cube-chaos-mod-setup`'s file-naming section (the paragraph right after the `IsUpgradeFrom:` rule) for the real incident this was caught from and why retrofitting it costs more (moving both DSL text and sprite pixels) than deciding it up front. Also add the matching `Tag: <Category>` line to the mod's `Description` file in the same edit (see that skill's `Tag:` section) — a category getting its own file and getting its own Workshop tag are two halves of the same signal.
