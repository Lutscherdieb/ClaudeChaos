...
- Egg perk uses **`FreeCopy`**, baby upgrade uses **`CopyOfCube`** — mirror the base game exactly. **Evidence (2026-07-25 audit):** every base-game Egg perk uses `FreeCopy`, no exceptions found. The baby upgrade is genuinely split in the base game itself — `Characters/Classes/ZUpgradeClassPerks.c.txt`'s 10 `Baby_X_Dragon` perks are 6 `FreeCopy` (Temporal/Iron/Holy/Explosive/Invasive/Anger) vs. 4 `CopyOfCube` (Magic/Icy/Programmed/Robot), and all 10 underlying `Baby_X_Dragon` cubes cost the identical 5 mana regardless of which function grants them — so this isn't a cost-dependent choice, it's a real inconsistency upstream that the base game itself ships with, both variants described identically as "Start with a free ... in your hand." This repo's own 3 lines (DJ/General/Unholy) all use `CopyOfCube` for the baby upgrade, which matches the `CopyOfCube`-using minority of real dragons exactly — **not a bug**, confirmed when a full-mod audit first flagged it as one. Don't "fix" these to `FreeCopy` without asking first; if a future session wants every line as reliably free instead of matching this specific base-game inconsistency, that's a deliberate call for the user, not a correction.
